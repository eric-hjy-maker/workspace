...
<img alt="系统层面的五种状态" src="images/.review_images/4e6858ed.png"/>
...
# JUC
##### 讲一下 ThreadLocal
    ThreadLocal 主要是在一个线程内传值的作用，对于不同线程的存储不同的值
    原理：表面上调用的是 ThreadLocal 的 set 方法，实际上放在当前线程的 ThreadLocalMap 里边，key 是 ThreadLocal，value 是 set 的值，所以不同线程的值不会受影响
    这里边还存在一个内存泄露的问题，key 的内存泄露，这里用了弱引用来解决，value还是一个强引用
    那么，jdk在这里做了一个优化，当 ThreadLocal put set时，检查那些 key 为 null 的 entry，然后清除掉
...
##### 事务的隔离级别？
    ...
    4. 串行化：不会出现幻读
##### 事务的四大特性？
    1. A 原子性：靠undo log实现，事务失败时，恢复到之前的状态
    2. C 一致性：靠其他几个特性实现
    3. I 隔离性：靠MVCC实现
    4. D 持久性：靠redo log实现，事务成功，就做 redo 操作，将所有操作写入磁盘
##### mysql 可重复读隔离级别下，仍然会出现的问题？
    会出现幻读
    不会出幻读的场景：普通情况下 select * from t 读的是快照，不会出现幻读
    除了快照读，mysql还有当前读，会在可重复读级别下，读到最新数据（有点像隔离级别降级了，处于读已提交级别）
    快照读的场景：除了普通的 select * from t 之外，如 select * from t lock in share mode,都是当前读
    出现幻读的场景：可重复读级别下，当前读，会出现幻读
    参考资料：https://blog.csdn.net/qq_42799615/article/details/110942949
    另外一种更可信的说法：
    幻读：两个事务先后插入一条数据，A事务插入成功后，B事务提示已存在，如同出现幻觉一样
    解决幻读就是进一步序列化事务：
    方式一：提升数据库隔离级别-序列化
    方式二：给记录加上锁，本质上也是让多个事务顺序执行
##### 查询方法需要使用事务吗？
    可重复读级别下：
    两条查询语句，需要使用事务，否则两个读出来数据不一样
##### mysql 使用的索引数据结构？
